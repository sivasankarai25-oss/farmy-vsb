...
export const AuthPage: React.FC<AuthPageProps> = ({ onAuthSuccess, language = 'en', onLanguageChange }) => {
  ...
  return (
    <div className="relative min-h-screen w-full flex items-center justify-center p-3 sm:p-6 select-none overflow-x-hidden bg-[#041209]">
      ...
      {/* Language Switcher (always visible, even before sign-in) */}
      {onLanguageChange && (
        <div className="absolute top-3 right-3 sm:top-5 sm:right-5 z-20">
          <button
            type="button"
            onClick={() => setShowLangMenu(v => !v)}
            className="flex items-center gap-1.5 bg-[#0c2217]/90 hover:bg-[#0c2217] text-emerald-100 text-xs font-semibold px-3 py-1.5 rounded-full border border-emerald-700/50 backdrop-blur-md transition-colors"
          >
            <span>🌐</span>
            <span>{LANGUAGE_OPTIONS.find(l => l.code === language)?.nativeLabel || 'English'}</span>
          </button>
          {showLangMenu && (
              ...
              {LANGUAGE_OPTIONS.map(opt => (
                <button
                  ...
                  className={`w-full text-left px-3 py-2 text-xs flex items-center justify-between hover:bg-emerald-50 ${
                    language === opt.code ? 'font-bold text-emerald-700 bg-emerald-50' : ''
                  }`}
                >
                  <span>{opt.nativeLabel}</span>
                  {language === opt.code && <Check className="w-3.5 h-3.5 text-emerald-600" />}
                </button>
              ))}
            </div>
          )}
        ...
        {(authMode === 'login' || authMode === 'register') && (
          <div className="bg-[#07190f]/90 p-1 rounded-2xl border border-emerald-900/60 flex items-center mb-5 relative">
            ...
            >
              ...
              <span>{t.authEmailSignInTab}</span>
            </button>
          ...
          <form onSubmit={handleEmailSignIn} className="space-y-4">
            ...
            <div className="flex items-center justify-between text-xs pt-0.5">
              <label className="flex items-center space-x-2 cursor-pointer text-emerald-200/90 hover:text-white">
                ...
                />
                <span className="font-medium">{t.authRememberMe}</span>
              </label>
          ...
          <form onSubmit={handleForgotPassword} className="space-y-4">
            ...
            <div className="text-center pt-2">
              ...
              >
                ...
                <span>{t.authReturnToSignIn}</span>
              </button>
            </div>
          </form>
        )}
